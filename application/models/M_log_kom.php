<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class M_log_kom extends CI_Model
{

    public $table = 'tb_log_kom';
    public $id = 'id_log';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id_log', $q);
	$this->db->or_like('id_komputer', $q);
	$this->db->or_like('nama_user', $q);
	$this->db->or_like('kerusakan', $q);
	$this->db->or_like('tindakan', $q);
	$this->db->or_like('ket', $q);
	$this->db->or_like('bagian', $q);
	$this->db->or_like('nama_it', $q);
	$this->db->or_like('tgl_mulai', $q);
	$this->db->or_like('tgl_selesai', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id_log', $q);
	$this->db->or_like('id_komputer', $q);
	$this->db->or_like('nama_user', $q);
	$this->db->or_like('kerusakan', $q);
	$this->db->or_like('tindakan', $q);
	$this->db->or_like('ket', $q);
	$this->db->or_like('bagian', $q);
	$this->db->or_like('nama_it', $q);
	$this->db->or_like('tgl_mulai', $q);
	$this->db->or_like('tgl_selesai', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }

}

/* End of file M_log_kom.php */
/* Location: ./application/models/M_log_kom.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2021-03-16 07:59:14 */
/* http://harviacode.com */